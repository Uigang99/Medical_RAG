...
SHARED_MARGIN_MODEL_VERSION = "rag2_shared_text_margin_regressor_v1"
...
@dataclass(frozen=True)
class SharedMarginRegressorConfig:
    """Configuration for the shared no-evidence/evidence margin regressor."""
    ...
    model_version: str = SHARED_MARGIN_MODEL_VERSION


class SharedTextMarginRegressor(nn.Module):
    """Predict a raw gold-vs-strongest-wrong margin from text.

    The exact same encoder and scalar head process both input conditions:
    ``Question+Options+[NO EVIDENCE]`` predicts m0, while
    ``Question+Options+Document`` predicts mD.  Gold answers and teacher
    margins are supervision only and are never accepted by ``forward``.
    """

    def __init__(self, config: SharedMarginRegressorConfig) -> None:
        super().__init__()
        self.margin_config = config
        self.encoder = T5EncoderModel.from_pretrained(config.base_model_name_or_path)
        encoder_size = int(self.encoder.config.d_model)
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> dict[str, torch.Tensor]:
        encoded = self.encoder(input_ids=input_ids, attention_mask=attention_mask)
        pooled = self.mean_pool(encoded.last_hidden_state, attention_mask)
        scaled_margin = self.regression_head(pooled).squeeze(-1)
        margin = scaled_margin * float(self.margin_config.margin_scale)
        return {"margin": margin, "scaled_margin": scaled_margin}
    ...
    def save_trainable(self, output_dir: Path) -> None:
        output_dir.mkdir(parents=True, exist_ok=True)
        (output_dir / "shared_margin_regressor_config.json").write_text(
            json.dumps(asdict(self.margin_config), ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
        save_file(self.trainable_state_dict(), str(output_dir / "trainable_model.safetensors"))
    ...
    def from_trainable(
        ...
        *,
        ...
        config = SharedMarginRegressorConfig(
            **json.loads((path / "shared_margin_regressor_config.json").read_text(encoding="utf-8"))
        )
        model = cls(config)
        state = load_file(str(path / "trainable_model.safetensors"), device=str(map_location))
        missing, unexpected = model.load_state_dict(state, strict=False)
        ...
        if missing_trainable or unexpected:
            raise RuntimeError(
                f"Invalid shared-margin checkpoint: missing={missing_trainable} unexpected={unexpected}"
            )
        return model

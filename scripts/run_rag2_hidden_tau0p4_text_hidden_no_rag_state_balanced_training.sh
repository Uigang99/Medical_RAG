...
OUTPUT_ROOT=/home/user/Uiheon/models/RAG2-Filter-FlanT5-large-HiddenUtilityTau0p4-NoRAGStateBalanced
RUN_NAME="${DATASET}_tau0p4_text_hidden_no_rag_state_balanced_epoch5"
...
"$PYTHON" "$PROJECT/scripts/train_rag2_hidden_feature_filter.py" \
  ...
  --run-name "$RUN_NAME" \
  --train-balance-mode no_rag_state_loss \
  --balanced-validation \
  --num-train-epochs 5 \
  --learning-rate 3e-5 \

...
OUTPUT_ROOT="/home/user/Uiheon/models/RAG2-PairwiseUtilityRegressor-FlanT5-large"
...
QUESTIONS_PER_BATCH="${TRAIN_QUESTIONS_PER_BATCH:-16}"
...
echo "[$(date '+%F %T')] Overall pairwise pilot 0/${TOTAL_STAGES}; overall ETA unknown"
...
for DATASET in "${DATASETS[@]}"; do
  ...
  echo "[$(date '+%F %T')] Stage ${STAGE}/${TOTAL_STAGES}: ${DATASET} pairwise-u pilot; overall elapsed=$(format_seconds "${ELAPSED}") overall ETA=${ETA_TEXT}"
  ...
  "${PYTHON_BIN}" \
    ...
    --run-name "${DATASET}_pointwise_pairwise_u_tau${UTILITY_THRESHOLD}_pilot_q${TRAIN_QUESTIONS}_qbatch${QUESTIONS_PER_BATCH}_epoch${EPOCHS}" \
    --num-train-epochs "${EPOCHS}" \
    --max-train-questions "${TRAIN_QUESTIONS}" \
    --max-eval-questions "${EVAL_QUESTIONS}" \
    --train-documents-per-batch 128 \
    --eval-documents-per-batch "${EVAL_BATCH}" \
    --train-questions-per-batch "${QUESTIONS_PER_BATCH}" \
    ...
    --checkpoint-metric within_question_spearman \
    --pairwise-ranking \
...
FINISHED="$(date +%s)"
echo "[$(date '+%F %T')] Overall pairwise pilot ${TOTAL_STAGES}/${TOTAL_STAGES} complete; elapsed=$(format_seconds "$((FINISHED - PIPELINE_START))") ETA=0s"

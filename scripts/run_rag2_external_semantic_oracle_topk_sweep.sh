#!/usr/bin/env bash
# Evaluate two GPT-5.6-Terra semantic gold-oracle policies on the exact external
# MCQ rerank Top-32 that was annotated:
#   1) direct_support only
#   2) direct_support + supporting_evidence
#
# Retrieval and reranking are never recomputed. Every Top-k condition is a
# prefix of the immutable annotated global rerank Top-32. There is no backfill.

set -euo pipefail

# Physical GPU 1 is the default. It is exposed as logical cuda:0 inside the job.
...
SEMANTIC_RUN_ROOT="${SEMANTIC_RUN_ROOT:-/home/user/codex_rag2_outputs/codex_evidence_utility_labels_external_oracle_top32_terra_medium_v1}"
PREPARED_ROOT="${PREPARED_ROOT:-$SEMANTIC_RUN_ROOT/_prepared_candidates}"
SEMANTIC_LABEL_ROOT="${SEMANTIC_LABEL_ROOT:-$SEMANTIC_RUN_ROOT/_annotation/terra_medium}"
ORACLE_LABELS="${ORACLE_LABELS:-$SEMANTIC_LABEL_ROOT/semantic_oracle_labels_top32.jsonl}"
ORACLE_LABEL_MANIFEST="${ORACLE_LABEL_MANIFEST:-$SEMANTIC_LABEL_ROOT/semantic_oracle_labels_top32_manifest.json}"

RESULTS_ROOT="${RESULTS_ROOT:-$PROJECT/results/rag2_llama3_paper_compatible_three_anchor_v1/all_mcq_global_rerank_top32_semantic_gold_oracle}"
...
test -f "$PREPARED_ROOT/prepare_manifest.json"
test -f "$SEMANTIC_LABEL_ROOT/manifest.json"
test -f "$SEMANTIC_LABEL_ROOT/external_oracle_top32_verification_report.json"
...
announce_stage 1 "export exact semantic decisions to evaluator identities"
"$PYTHON" "$PROJECT/scripts/materialize_rag2_external_oracle_semantic_candidates.py" \
  export-oracle \
  --prepared-root "$PREPARED_ROOT" \
  --label-root "$SEMANTIC_LABEL_ROOT" \
...
for TOP_K in "${TOP_K_VALUES[@]}"; do
  for POLICY in "${POLICIES[@]}"; do
    ...
    announce_stage "$STAGE_INDEX" "$POLICY: annotated rerank prefix Top-$TOP_K"
...
announce_stage 14 "validate all conditions and write one comparison table"

...
class WorkflowProgress:
    ...
    def _status(self) -> None:
        now = time.time()
        completed_now = self.stage_done
        elapsed = max(now - self.stage_started, 1e-6)
        rate = max(0.0, completed_now / elapsed)
        fraction = completed_now / self.stage_total
        stage_eta = (self.stage_total - completed_now) / rate if rate > 0 else None
...
class DirectChoicePcedScorer:
    ...
    def sequences(
        ...
        for sample, row in zip(samples, rows, strict=True):
            ...
            base.append(sequence_for_prompt(self.tokenizer, sample, base_text)[0])
            experts.extend(
                sequence_for_prompt(self.tokenizer, sample, str(document["text"]).strip())[0]
    ...
    def hidden(self, sequences: Sequence[Sequence[int]]) -> torch.Tensor:
        ...
        for start in range(0, len(order), self.args.prompt_batch_size):
            indices = order[start : start + self.args.prompt_batch_size]
    ...
    def score_questions(
        ...
        no_sequences, base_sequences, expert_sequences = self.sequences(samples, rows)
...
def aggregate(
    ...
    for sample, candidate in zip(samples, candidate_rows, strict=True):
        ...
        predictions = {
            ...
            "base_rag_top8": CHOICES[int(base_logits.argmax())],
        }
...
def main() -> None:
    ...
    progress = WorkflowProgress(
        [
            ...
            "score Top-8 Semantic-Support probabilities",
            "score no-RAG, concatenated Top-8, and eight independent Llama experts",
    ...
    try:
        ...
        for sample, row in zip(samples[:audit_count], candidates[:audit_count], strict=True):
            ...
            texts.extend(str(document["text"]).strip() for document in docs)
            maximum = max(maximum, *(len(sequence_for_prompt(tokenizer, sample, text)[0]) for text in texts))
        del tokenizer
        if maximum > args.max_input_tokens:
            raise RuntimeError(f"Sample prompt exceeds max input tokens: {maximum}")
        progress.update(1)
        progress.complete(
            f"questions={len(samples)} documents={len(samples)*args.top_k} sampled_max_tokens={maximum} "

...
OUTPUT_ROOT=/home/user/Uiheon/models/RAG2-Filter-FlanT5-large-HiddenUtilityTau0p4
RUN_NAME="${DATASET}_tau0p4_text_hidden_epoch5"

if [[ ! -f "$SPLIT_ROOT/$DATASET/manifest.json" ]]; then
  echo "Missing prepared split: $SPLIT_ROOT/$DATASET/manifest.json" >&2
  echo "Run $PROJECT/scripts/prepare_rag2_hidden_tau0p4_binary_filter_data.sh first." >&2
  exit 1
fi

...
OUTPUT_DIR="${OUTPUT_DIR:-$PROJECT/results/rag2_pced_history_rationale_answer_top8_pilot_v1}"
...
echo "[cache policy] exact legacy outputs are reused only after exact fidelity; every new condition is atomically sharded and resumes with this command"
...
echo "[cold-run estimate] exact legacy match: about 25-55 minutes; mismatch and all five conditions regenerated: about 50-110 minutes on one H200. The active stage reports measured ETA after its first batch."
